...
- **Portfólio e depoimentos:** só com casos reais e autorizados. Os exemplos "ilustrativos" antigos foram retirados de propósito.
...
- **Rotas e i18n:** `prefixDefaultLocale: false`, sem fallback.
  ...
  - As seções bilíngues (`Hero`, `ComoFunciona`, `Sobre`, `Servicos`, `CtaFinal`, `Tech`) recebem a prop `lang` e guardam seus próprios textos `pt`/`en` com o mesmo padrão `typeof pt`.
...
- **Site antigo até o cutover de DNS:** o bundle antigo (`index.html`) saiu desta branch, mas continua na `main`, que o GitHub Pages serve no domínio.
  ...
  - Veja o site com `npm run dev` (localhost:4321). O Live Server do VS Code (porta 5500) só serve arquivos estáticos da raiz e não renderiza Astro.
...
Gerenciador de pacotes: **npm**. O lockfile é o `package-lock.json`. Não use yarn nem pnpm neste projeto.

```bash
npm install          # instalar dependências (o prepare instala os hooks do Husky)
npm run dev          # astro dev (de um agente: `npx astro dev --background` + `astro dev stop|logs`)
npm run build        # astro check (tipos) && astro build → dist/
npm run preview      # wrangler dev: serve o dist/ com o roteamento de produção (307 de barra final, 404, _headers)
npm run deploy       # deploy manual (normalmente o Workers Builds publica no push da main)
npm run lint         # ESLint (npm run lint:fix corrige o que der)
npm run format       # Prettier em tudo (npm run format:check só confere)
npm run typecheck    # astro check sozinho
npm run knip         # arquivos, exports e dependências não usados
npm run secretlint   # procura tokens e chaves no código
npm run seo          # confere o ./dist contra as regras de seo.md (rode depois do build)
```
